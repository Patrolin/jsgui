"use strict";
function generateFontSizeCssVars(fontSizesStart, sizes) {
    if (fontSizesStart === void 0) { fontSizesStart = [13]; }
    if (sizes === void 0) { sizes = SIZES; }
    var getSize = function (fontSize) { return Math.ceil(1.5 * fontSize); };
    var fontSizes = fontSizesStart;
    for (var i = fontSizesStart.length; i < sizes.length; i++) {
    ...
    for (var i = 0; i < sizes.length; i++) {
        ...
        var iconSize = 1.25 * fontSize;
        var size = getSize(fontSize);
        acc += "\n  --fontSize-".concat(sizeName, ": ").concat(fontSize, "px;");
        acc += "\n  --iconSize-".concat(sizeName, ": ").concat(iconSize, "px;");
        acc += "\n  --size-".concat(sizeName, ": ").concat(size, "px;");
    }
    return acc;
}
