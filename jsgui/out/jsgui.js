/* generateFontVars.ts */
window.generateFontSizeCssVars = function generateFontSizeCssVars(names/*: string[] */= SIZES) {
  /* type SizeDef = {
    fontSize: number;
    iconSize: number;
    size: number;
...
window.generateColorCssVars = function generateColorCssVars(colors/*: StringMap<string> */= BASE_COLORS, start = 0.874, step = 2, shades = COLOR_SHADES) {
...
// utils
window.JSGUI_VERSION = "v0.8";
window.parseJsonOrNull = function parseJsonOrNull(jsonString/*: string*/)/*: JSONValue */{
  try {
...
window.camelCaseToKebabCase = function camelCaseToKebabCase(key/*: string*/) {
  return (key.match(/[A-Z][a-z]*|[a-z]+/g) ?? []).map(v => v.toLowerCase()).join("-");
}
window.removePrefix = function removePrefix(value/*: string*/, prefix/*: string*/)/*: string */{
  return value.startsWith(prefix) ? value.slice(prefix.length) : value;
}
window.removeSuffix = function removeSuffix(value/*: string*/, prefix/*: string*/)/*: string */{
  return value.endsWith(prefix) ? value.slice(value.length - prefix.length) : value;
}
window.addPx = function addPx(value/*: string | number*/) {
  return (value?.constructor?.name === "Number") ? `${value}px` : value /* as string */;
}
window.lerp = function lerp(value/*: number*/, x/*: number*/, y/*: number*/)/*: number */{
  return (1-value)*x + value*y;
}
/** clamp value between min and max (defaulting to min) */
window.clamp = function clamp(value/*: number*/, min/*: number*/, max/*: number*/)/*: number */{
  return Math.max(min, Math.min(value, max));
}
window.rgbFromHexString = function rgbFromHexString(hexString/*: string*/)/*: string */{
...
window.getDiff = function getDiff/* <T> */(oldValue/*: StringMap<T>*/, newValue/*: StringMap<T>*/)/*: Diff<T>[] */{
...
window.getDiffArray = function getDiffArray(oldValues/*: string[]*/, newValues/*: string[]*/)/*: Diff<string>[] */{
...
// component
window._setChildKey = function _setChildKey(component/*: Component*/, child/*: Component*/) {
...
window.makeComponent = function makeComponent/* <A extends Parameters<any>> */(onRender/*: RenderFunction<A>*/, options/*: ComponentOptions */= {})/*: ComponentFunction<A> */{
...
window._copyRootComponent = function _copyRootComponent(component/*: Component*/) {
...
// dispatch
window.DispatchTarget = class DispatchTarget {
...
window.DispatchTargetMap = class DispatchTargetMap {
...
window._scrollToLocationHash = function _scrollToLocationHash() {
  const element = document.getElementById(location.hash.slice(1));
  if (element) element.scrollIntoView();
}

// metadata
window.ComponentMetadata = class ComponentMetadata {
  // state
...
window.RootComponentMetadata = class RootComponentMetadata extends ComponentMetadata {
  component: Component;
...
// render
window.renderRoot = function renderRoot(rootComponent: Component, parentNode: ParentNodeType | null = null): RootComponentMetadata {
...
window._START_BASE_PROPSNone/*: InheritedBaseProps */= {
  attribute: {},
  className: [],
  cssVars: {},
  style: {},
};
window._render = function _render(component/*: Component*/, parentNode/*: ParentNodeType*/, beforeNodeStack/*: (NodeType | null)[] */= [], _inheritedBaseProps/*: InheritedBaseProps */= _START_BASE_PROPS, isTopNode = true) {
  // render elements
...
// rerender
window._unloadUnusedComponents = function _unloadUnusedComponents(prevComponent/*: Component*/, rootGcFlag/*: boolean*/) {
...
window.moveRoot = function moveRoot(root_/*: RootComponentMetadata*/, parentNode/*: ParentNodeType*/) {
  root_.parentNode = parentNode;
  if (root_.prevNode) parentNode.append(root_.prevNode);
}
window.unloadRoot = function unloadRoot(root_/*: RootComponentMetadata*/) {
  _unloadUnusedComponents(root_.component, !root_.gcFlag);
}

// basic components
window.fragment = makeComponent(function fragment(_props/*: BaseProps */= {}) {}, { name: '' });
window.text = makeComponent(function text(str/*: string*/, _props/*: {} */= {}) {
...
window.ul = makeComponent(function ul(_props/*: BaseProps */= {}) {
  this.useNode(document.createElement("ul"));
});
window.ol = makeComponent(function ol(_props/*: BaseProps */= {}) {
  this.useNode(document.createElement("ol"));
});
window.li = makeComponent(function li(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("li"));
  this.append(text);
});
window.h1 = makeComponent(function h1(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h1"));
  this.append(text);
});
window.h2 = makeComponent(function h2(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h2"));
  this.append(text);
});
window.h3 = makeComponent(function h3(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h3"));
  this.append(text);
});
window.h4 = makeComponent(function h4(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h4"));
  this.append(text);
});
window.h5 = makeComponent(function h5(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h5"));
  this.append(text);
});
window.h6 = makeComponent(function h6(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("h6"));
  this.append(text);
});
window.divider = makeComponent(function divider(vertical/*: boolean */= false, _props/*: BaseProps */= {}) {
  this.append(div({
    attribute: {dataVertical: vertical},
  }));
});
window.p = makeComponent(function p(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("p"));
  this.append(text);
});
window.b = makeComponent(function b(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("b"));
  this.append(text);
});
window.em = makeComponent(function em(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("em"));
  this.append(text);
});
window.code = makeComponent(function code(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("code"));
  this.append(text);
});
window.button = makeComponent(function button(text/*: string*/, _props/*: BaseProps */= {}) {
  this.useNode(document.createElement("button"));
  this.append(text);
});
window.input = makeComponent(function input(_props/*: BaseProps */= {}) {
  this.useNode(document.createElement("input"));
});
window.svg = makeComponent(function svg(svgText/*: string*/, _props/*: BaseProps */= {}) {
...
window.div = makeComponent(function div(_props/*: BaseProps */= {}) {
  this.useNode(document.createElement('div'));
});
/* type Size = "small" | "normal" | "big" | "bigger"; */
window.SIZESNone/*: Size[] */= ["small", "normal", "big", "bigger"];
/* type BaseColor = "gray" | "secondary" | "red"; */
window.BASE_COLORSNone/*: Record<BaseColor, string> */= {
  gray: "0, 0, 0",
  secondary: "20, 80, 160",
  red: "200, 50, 50",
};
window.COLOR_SHADES = ["", "033", "067", "1", "2", "250", "3"]; // TODO: use linear colors
...
window.span = makeComponent(function _span(text/*: string | number | null | undefined*/, props/*: SpanProps */= {}) {
...
window.icon = makeComponent(function icon(iconName/*: string*/, props/*: IconProps */= {}) {
  let {size, style = {}, ...extraProps} = props;
  this.baseProps.attribute.dataIcon = iconName;
  this.append(span("", {iconName, size, style, ...extraProps}));
});
window.loadingSpinner = makeComponent(function loadingSpinner(props/*: IconProps */= {}) {
  this.append(icon("progress_activity", props));
});
window.legend = makeComponent(function legend(text/*: string*/, _props/*: BaseProps */= {}) {
...
window.dialog = makeComponent(function dialog(props/*: DialogProps*/)/*: RenderReturn */{
...
window._computeScrollbarWidth = function _computeScrollbarWidth() {
...
window._getPopupLeftTop = function _getPopupLeftTop(direction/*: PopupDirection*/, props/*: {
...
window._getPopupLeftTopWithFlipAndClamp = function _getPopupLeftTopWithFlipAndClamp(props/*: {
...
window.popupWrapper = makeComponent(function popupWrapper(props/*: PopupWrapperProps*/)/*: RenderReturn */{
...
// inputs
window.coloredButton = makeComponent(function coloredButton(text: string, props: ButtonProps = {}) {
...
window.controlledInput = makeComponent(function controlledInput(props/*: InputProps*/) {
...
window.labeledInput = makeComponent(function labeledInput(props/*: LabeledInputProps*/) {
...
window.errorMessage = makeComponent(function errorMessage(error/*: string*/, props/*: SpanProps */= {}) {
...
window.textInput = makeComponent(function textInput(props/*: TextInputProps*/) {
...
window.numberArrows = makeComponent(function numberArrows(props/*: NumberArrowProps */= {}) {
...
window.numberInput = makeComponent(function numberInput(props/*: NumberInputProps*/) {
  const {
...
window.table = makeComponent(function table(props/*: TableProps & BaseProps*/) {
...
window.router = makeComponent(function router(props/*: RouterProps*/) {
  const {
    routes,
